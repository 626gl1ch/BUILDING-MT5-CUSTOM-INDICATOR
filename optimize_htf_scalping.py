...
def generate_ema_pullback(df, p):
    ...
    ema_trend_p = p.get('ema_trend_p', 50)
    ema_htf = df.get(f'ema_{ema_trend_p}', close)
...
def generate_mean_reversion_filtered(df, p):
    ...
    ema_trend_p = p.get('ema_trend_p', 50)
    ema_trend = df.get(f'ema_{ema_trend_p}', close)
...
def generate_connors_rsi(df, p):
    ...
    ema_trend_p = p.get('ema_trend_p', 50)
    ema_200 = df.get(f'ema_{ema_trend_p}', close)
    signals = pd.Series(0, index=df.index)
...
def main():
    ...
    print("\n[1/3] Loading HTF Historical Datasets...")
    data_htf = {}
    
    data_15 = engine.load_all_data(suffix="15min_1year")
    for k, v in data_15.items():
        data_htf[f"{k}_15m"] = v
        
    data_30 = engine.load_all_data(suffix="30min_1year")
    for k, v in data_30.items():
        data_htf[f"{k}_30m"] = v
    
    print("\n[2/3] Precomputing 150+ Indicators...")
    precomputed = {}
    for sym, df in data_htf.items():
        print(f"  Precomputing indicators for {sym}...")
        precomputed[sym] = add_all_indicators(df)
        
    search_grids = [
        {'name': 'EMA Pullback Scalper', 'fn': generate_ema_pullback, 'grid': {'fast_ema': [5, 10], 'slow_ema': [50], 'rsi_period': [14], 'rsi_pullback': [40, 50], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True], 'ema_trend_p': [50, 200]}},
        {'name': 'Volatility Expansion Breakout', 'fn': generate_volatility_breakout, 'grid': {'donchian_p': [20], 'bb_width_max': [0.04], 'chop_max': [45.0], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True], 'ema_trend_p': [50, 200]}},
        {'name': 'Filtered Mean Reversion', 'fn': generate_mean_reversion_filtered, 'grid': {'bb_p': [20], 'z_thresh': [2.0], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True], 'ema_trend_p': [50, 200]}},
        {'name': 'SMA Crossover Scalper', 'fn': generate_ma_crossover, 'grid': {'ma_type': ['sma'], 'fast_p': [10], 'slow_p': [50], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'EMA Crossover Scalper', 'fn': generate_ma_crossover, 'grid': {'ma_type': ['ema'], 'fast_p': [10], 'slow_p': [50], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'MACD Strategy', 'fn': generate_macd_strategy, 'grid': {'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'Stoch RSI Strategy', 'fn': generate_stoch_rsi, 'grid': {'os_lvl': [20], 'ob_lvl': [80], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'KAMA Strategy', 'fn': generate_kama_strategy, 'grid': {'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'Connors RSI Strategy', 'fn': generate_connors_rsi, 'grid': {'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True], 'ema_trend_p': [50, 200]}},
        {'name': 'DC Strategy', 'fn': generate_dc_breakout, 'grid': {'donchian_p': [20], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'MFI Strategy', 'fn': generate_mfi_strategy, 'grid': {'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True], 'ema_trend_p': [50, 200]}},
        {'name': 'SMASIG Strategy', 'fn': generate_smasig_strategy, 'grid': {'sma_p': [20], 'sig_p': [5], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'SMA Close/Exit Signals', 'fn': generate_sma_close_exit, 'grid': {'sma_p': [20], 'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'OVUN Strategy', 'fn': generate_ovun_strategy, 'grid': {'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'FX Replay Session Breakout', 'fn': generate_fxreplay_session, 'grid': {'sl_atr': [1.0, 2.0], 'tp_atr': [3.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [True]}},
        {'name': 'SMC Liquidity Sweep', 'fn': generate_smc_liquidity_sweep, 'grid': {'lookback': [14, 20], 'sl_atr': [0.5, 1.0], 'tp_atr': [2.0, 5.0], 'max_bars_hold': [9999], 'risk_pct': [0.01], 'trailing': [False, True]}}
    ]
        ...
        for idx, c in enumerate(combos):
            try:
                res = engine.run_full_validation(
                    ...
                    slippage_pct=0.00005, fee_pct=0.00005
                )
    ...
    report_file = "SCALPING_HTF_STRATEGY_VALIDATION_REPORT.md"
