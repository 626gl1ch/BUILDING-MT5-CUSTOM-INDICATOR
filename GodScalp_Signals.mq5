//+------------------------------------------------------------------+
//|                                            GodScalp_Signals.mq5  |
//|                                   GOD SCALP v2 - Glitch EnterPrice|
//+------------------------------------------------------------------+
...
input double InpSlopeNoiseFrac            = 0.05;   // slope must exceed this fraction of ATR14 to count as real
...
input int    InpPendingMaxBars            = 20;       // bars a pending setup stays armed
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "GOD SCALP Signals v2.1");
...
void ProcessClosedBar(int i,bool isLive)
{
   ...
   bool fueled = true; // Fuel filter removed

   int i3 = MathMax(0,i-3);
   double slope5  = BufAlma5[i]-BufAlma5[i3];
   ...
   if(g_pendActive)
   {
      ...
      if(g_pendMode==0)
      {
         double slopeFloor = InpSlopeNoiseFrac*atrF;
         if(g_pendDir==1) stillValid = (slope5>slopeFloor);
         else             stillValid = (slope5<-slopeFloor);
      }
      if(stillValid && fueled)
      {
         if(g_pendDir==1 && macdCrossUp)
         {
            bool ok = FireEntry(i,1,g_pendMode,isLive);
   ...
   if(!fueled){ g_pendActive=false; return; }
   ...
   {
      double slopeFloor = InpSlopeNoiseFrac*atrF;
      bool ltfUp   = (slope5>slopeFloor);
      bool ltfDown = (slope5<-slopeFloor);

      if(ltfUp)
      {
         bool touched  = (g_low[i]<=BufBBBasis[i] || g_low[i]<=BufBBLower[i]);
         bool candleOk = (g_close[i]>g_open[i]);
         if(touched && candleOk)
         {
            if(macdCrossUp)
            {
               bool ok = FireEntry(i,1,0,isLive);
               if(isLive){ if(ok) IncFired(); else IncRejectedRR(); }
            }
      ...
      if(ltfDown)
      {
         bool touched  = (g_high[i]>=BufBBBasis[i] || g_high[i]>=BufBBUpper[i]);
         bool candleOk = (g_close[i]<g_open[i]);
         if(touched && candleOk)
         {
            if(macdCrossDown)
...
bool FireEntry(int i,int dir,int mode,bool isLive)
{
   ...
   if(isLive || i > ArraySize(g_time) - 1000)
   {
      ...
      ObjectCreate(0, objPrefix+"SL", OBJ_TREND, 0, tStart, sl, tEnd, sl);
      ObjectSetInteger(0, objPrefix+"SL", OBJPROP_COLOR, clrCrimson);
      ObjectSetInteger(0, objPrefix+"SL", OBJPROP_WIDTH, 2);
      ObjectSetInteger(0, objPrefix+"SL", OBJPROP_RAY_RIGHT, false);
      ObjectSetInteger(0, objPrefix+"SL", OBJPROP_BACK, true);
      
      ObjectCreate(0, objPrefix+"TP", OBJ_TREND, 0, tStart, t1, tEnd, t1);
      ObjectSetInteger(0, objPrefix+"TP", OBJPROP_COLOR, clrLimeGreen);
      ObjectSetInteger(0, objPrefix+"TP", OBJPROP_WIDTH, 2);
      ObjectSetInteger(0, objPrefix+"TP", OBJPROP_RAY_RIGHT, false);
      ObjectSetInteger(0, objPrefix+"TP", OBJPROP_BACK, true);
...
{
   if(!g_hasActiveTrade) return;
   SetHLine("GS_Entry", g_tradeEntryPrice, clrSilver, STYLE_DOT);
   SetHLine("GS_SL", g_tradeSL, clrCrimson, STYLE_SOLID);
   if(!g_tradePartialTaken)
   {
      SetHLine("GS_T1", g_tradeT1, clrLimeGreen, STYLE_DASH);
      ObjectDelete(0,"GS_Runner");
   }
   else
   {
      ObjectDelete(0,"GS_T1");
      SetHLine("GS_Runner", g_tradeRunner, clrDodgerBlue, STYLE_DASHDOT);
   }
}
                 ...
                 const int &spread[])
{
   ...
   int warmupBars = MathMax(MathMax(InpBBLen, InpChopLen), MathMax(InpAtrFast, InpMacdSlow)) + 5;
   ...
   ArrayResize(g_chop,total);     ArrayResize(g_regime,total);
   ArrayResize(g_alma5Slope,total);
   ...
   for(int i=start;i<total;i++)
   {
      ...
      int i3 = MathMax(0,i-3);
      g_alma5Slope[i] = BufAlma5[i]-BufAlma5[i3];
   }
